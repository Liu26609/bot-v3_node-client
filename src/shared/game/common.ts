   ...
   getSkDesc(info: SKILL_ACTIVE) {
      ...
      switch (info.type) {
         ...
         case SKILL_TYPE.upAllDefense_fixed:
            ...
            break;
         case SKILL_TYPE.resHp_fixed_all:
            temp += `每次触发回复我方全体${info.data[0]}生命`
            break;
         case SKILL_TYPE.realHurt_fixed:
            temp += `对一个敌人造成${info.data[0]}真实伤害`
            break;
         case SKILL_TYPE.creatorEnemy:
            temp += `召唤${info.data[1]}只${info.data[0]}级怪物参加战斗`
            break;
         case SKILL_TYPE.forever_add_hp:
            temp += `每触发一次有${info.data[1]}%概率永久生命+${info.data[0]}`
            break;
         case SKILL_TYPE.round_realHurt:
