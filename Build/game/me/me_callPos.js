...
class me_callPos extends task_base_1.task_base {
    ...
    menu() {
        let temp = `┏┄═════🌏传送═════━┄
┣传送指令：传送 + 坐标
┣每次传送需要${prop_1.rewardKey_CN[prop_1.rewardKey[prop_1.rewardKey.transferCard]]}x1
🌰栗子：传送 + 10，20(传送10,20)
🧚‍♂️提示：逗号不方便？可以试试这些符号(.。,，+-!！*)
┗┄━══════════━┄`;
    ...
    judePositionREG(str) {
        ...
        if (isNaN(Number(sendPos[0]))) {
            // 坐标不是数字
            bot_1.default.sendText(this.channel_id, `🧚‍♂️你发送的传送坐标不是一个数字噢~`);
            return false;
        }
        ...
        if (Number(sendPos[0]) < 0 || Number(sendPos[1]) < 0) {
            // 坐标不是数字
            bot_1.default.sendText(this.channel_id, `🧚‍♂️此坐标暂未开放~`);
            return false;
        }
        return sendPos;
    }
